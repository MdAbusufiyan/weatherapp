...
class WeatherApp(QWidget):
    ...
    def load_weather(self):
        ...
        self.weather_worker = WeatherWorker(self.lat, self.lon)
        ...
        self.weather_thread.finished.connect(lambda: setattr(self, "weather_thread", None))
    ...
    def search_city(self):
        ...
        # stop previous geo thread safely
        if self.geo_thread and self.geo_thread.isRunning():
            self.geo_thread.quit()
            self.geo_thread.wait()

        self.geo_thread = QThread()
        self.geo_worker = GeoWorker(city)
        self.geo_worker.moveToThread(self.geo_thread)

        self.geo_thread.started.connect(self.geo_worker.run)
        self.geo_worker.finished.connect(self.on_geo_ready)
        self.geo_worker.error.connect(self.on_weather_error)

        self.geo_worker.finished.connect(self.geo_thread.quit)
        self.geo_worker.finished.connect(self.geo_worker.deleteLater)
        self.geo_thread.finished.connect(self.geo_thread.deleteLater)
        self.geo_thread.finished.connect(lambda: setattr(self, "geo_thread", None))
