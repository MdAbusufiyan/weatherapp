...
from PyQt6.QtWidgets import (
    ...
    QFrame, QGraphicsDropShadowEffect, QGraphicsOpacityEffect, QGridLayout, QPushButton, QSizePolicy, QLineEdit, QCompleter, QGraphicsOpacityEffect
)
from PyQt6.QtCore import Qt, QTimer, QPropertyAnimation, QEasingCurve, QPoint, QRectF, QObject, QThread, pyqtSignal, QThreadPool, QRunnable
...
class AnalogClock(QWidget):
    def __init__(self):
        ...
        self.main_color = QColor(255,255,255)
        self.accent_color = QColor(0,217,255)
    ...
    def set_contrast(self, dark_mode):
        if dark_mode:
            self.main_color = QColor(20,20,20)
            self.accent_color = QColor(0,120,160)
        else:
            self.main_color = QColor(255,255,255)
            self.accent_color = QColor(0,217,255)
    ...
    def paintEvent(self, event):
        ...
        painter.setPen(QPen(self.main_color, 4, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
        painter.drawLine(
...
class MetricBar(QFrame):
    ...
    def set_value(self, value, suffix=""):
        ...
        self.suffix = suffix
        self.parent().parent().weather_display.metrics_animating = True
    ...
    def animate_value(self):
        ...
        if abs(diff) < 0.1:
            ...
            self.anim_timer.stop()
            self.parent().parent().weather_display.metrics_animating = False
        else:
...
class WeatherDisplay(QFrame):
    def __init__(self):
        ...
        # --- performance governor timer ---
        self.governor_timer = QTimer()
        self.governor_timer.timeout.connect(self.adjust_performance)
        self.governor_timer.start(1500)  # check every 1.5s
        ...
        location_row.addWidget(self.search_status)
        location_row.addWidget(self.search_btn)
        location_row.addWidget(self.refresh_btn)
    ...
    def set_weather_type(self, temp):
        ...
        if self.weather_type == "rain":
            calculated_count = int(60 * (self.width()/800 + 0.5))
            count = min(calculated_count, 120)
            ...
            calculated_count = int(25 * (self.width()/800 + 0.5))
            count = min(calculated_count, 120)
            for _ in range(count):
                self.particles.append(
                    Particle(
    ...
    def apply_contrast(self):
        ...
        # Condition
        self.condition.setStyleSheet(f"""
            font-size: 22px;
            color: {sub};
            font-weight: 300;
            letter-spacing: 2px;
        ...
        # Digital clock
        self.digital_clock.time_label.setStyleSheet(f"""
            font-size: 75px;
            font-weight: 700;
            color: {main};
            font-family: "Old English Text MT";
        ...
        self.digital_clock.date_label.setStyleSheet(f"""
            font-size: 18px;
            color: {sub};
            font-family: "Old English Text MT";
            background: transparent;
        ...
        self.clock.set_contrast(self.weather_type == "snow")
        # --- Search + Refresh buttons contrast ---
        ...
        """)
        # --- animate transition ---
        self.fade_widget(self.temperature)
        self.fade_widget(self.condition)
        self.fade_widget(self.location)
        self.fade_widget(self.digital_clock.time_label)
        self.fade_widget(self.digital_clock.date_label)
        self.fade_widget(self.search_btn)
        self.fade_widget(self.refresh_btn)
    ...
    def update_particles(self):
        ...
        # pause particles while metric animations are running
        if getattr(self, "metrics_animating", False):
            return
    ...
    def adjust_performance(self):
        # if window hidden or minimized → very low FPS
        if not self.isVisible() or self.window().isMinimized():
            self.particle_timer.start(150)   # ~6 FPS
            return

        # active window
        if self.weather_type in ["rain", "snow"]:
            self.particle_timer.start(40)    # ~25 FPS
        else:
            self.particle_timer.start(70)    # ~14 FPS
    ...
    def run(self):
        try:
            geo_url = (
                ...
                f"?q={self.city}&format=json&limit=1"
            )
            ...
            if res:
                lat = float(res[0]["lat"])
                lon = float(res[0]["lon"])
                self.finished.emit(lat, lon, self.city)
            else:
...
class WeatherApp(QWidget):
    def __init__(self):
        ...
        self.weather_request_active = False
        self.pending_weather_request = False
        ...
        self.weather_display.refresh_btn.clicked.connect(self.load_weather)
        ...
        self.net_timer.timeout.connect(self.check_connection)
        self.net_timer.start(20000)  # every 20 seconds
    ...
    def check_connection(self):
        try:
            ...
            # if previously offline, retry loading
            if not self.weather_request_active:
                self.load_weather()
        except:
            pass
    ...
    def load_weather(self):
        # if a request is already running, mark pending and return
        if self.weather_request_active:
            self.pending_weather_request = True
            return

        self.weather_request_active = True
        self.pending_weather_request = False

        worker = WeatherWorker(self.lat, self.lon)
        self.active_workers.append(worker)
        worker.finished.connect(
            lambda weather, geo, w=worker: self.on_weather_loaded(weather, geo, w)
        )
    ...
    def search_city(self):
        ...
        if not city:
            return
        self.weather_display.search_status.setText("SEARCHING...")
        self.weather_display.search_status.setVisible(True)


        worker = GeoWorker(city)
        self.active_workers.append(worker)
        worker.finished.connect(
            lambda lat, lon, city, w=worker: self.on_geo_ready(lat, lon, city, w)
        )
        worker.error.connect(self.on_weather_error)
    ...
    def on_weather_loaded(self, weather, geo, worker):
        self.weather_display.search_status.setVisible(False)
        ...
        # if another request was triggered while running, execute again
        if self.pending_weather_request:
            self.pending_weather_request = False
            QTimer.singleShot(10, self.load_weather)

        self.active_workers = [w for w in self.active_workers if w != worker]
 
    def on_geo_ready(self, lat, lon, city, worker):
        self.lat = lat
        self.lon = lon
    ...
    def on_weather_error(self, msg):
        ...
        self.weather_display.search_status.setText("OFFLINE — WAITING FOR CONNECTION")
        self.weather_display.search_status.setVisible(True)
