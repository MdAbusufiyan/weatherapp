...
class WeatherApp(QWidget):
    def __init__(self):
        ...
        self.suggest_timer.timeout.connect(self.fetch_suggestions)

        self.weather_display.search_input.textEdited.connect(
            lambda: self.suggest_timer.start(800)   # 800 ms debounce
        )
    ...
    def fetch_suggestions(self):
        text = self.weather_display.search_input.text().strip()

        # ignore short or numeric input
        ...
        try:
            url = (
                ...
                f"?q={text}&format=json&limit=5"
            )

            res = requests.get(url, headers={"User-Agent": "weather-app"}, timeout=3).json()

            suggestions = [r["display_name"] for r in res]
    ...
    def search_city(self):
        ...
        try:
            ...
            res = requests.get(geo_url, headers={"User-Agent": "weather-app"}, timeout=5).json()
            if res:
    ...
    def load_weather(self):
        try:
            ...
            geo = requests.get(geo_url, headers={"User-Agent": "weather-app"}, timeout=5).json()
            ...
            self.address_panel.set_address("Unable to load location data")
